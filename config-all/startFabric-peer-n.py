...
# ARCH=$ARCH docker-compose -f "${DIR}"/composer/docker-compose-kafka-peer""" + str(i + 2) + """.yml down
ARCH=$ARCH docker-compose -f "${DIR}"/composer/docker-compose-kafka-peer""" + str(i + 2) + """.yml up -d
"""

    text_file = open("startKafka-Peer" + str(i + 2) + ".sh", "w")
    text_file.write(file)
    text_file.close()